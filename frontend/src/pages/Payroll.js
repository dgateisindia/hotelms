...
const monthYearString = (d) => d.toLocaleString('en-US', { month: 'long', year: 'numeric' });
...
function Payroll() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const now = new Date();
      setPeriodDate(prev => (monthYearString(prev) === monthYearString(now) ? prev : now));
    }, 60 * 1000);
    return () => clearInterval(interval);
  }, []);

  const monthYear = monthYearString(periodDate);
  ...
  const fetchPayroll = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      setError('Could not load payroll records. Please try again.');
      setPayrollRows([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    fetch('/api/staff?status=Active')
      ...
      .then(data => setActiveStaffCount(Array.isArray(data) ? data.length : null))
      .catch(() => setActiveStaffCount(null));
  }, []);
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      alert(data.generated > 0
        ? `Generated payroll for ${data.generated} employee(s) for ${monthYear}.`
  ...
  const saveEdit = async () => {
    try {
      const res = await fetch(`/api/payroll/${selected.payroll_id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const markPaid = async (p) => {
    try {
      const res = await fetch(`/api/payroll/${p.payroll_id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    <>
      <div className="payroll-topbar">
        <div style={{display:'flex',alignItems:'center',gap:6}}>
          ...
          <button className="payroll-date-btn"><IcoCalendar/> {monthYear}</button>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={10} style={{textAlign:'center',padding:40,color:'#9ca3af'}}>
                No payroll records for {monthYear}. Click "Process Payroll" to generate them.
              </td></tr>
            ) : paginated.map((p) => (
              <tr key={p.payroll_id}>
