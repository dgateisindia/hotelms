...
function Attendance() {
  ...
  const [leaveError] = useState(true);
  const [birthdays] = useState([]);
  const [birthdayError] = useState(true);
  const [topPerformers] = useState([]);
  const [performanceError] = useState(true);
  ...
  const fetchAttendance = useCallback(async () => {
    ...
    setLoadError('');
    try {
      const res = await apiClient.get('/attendance', { params: { date } });
  ...
  const handleEdit = async () => {
    ...
    setSubmitting(true);
    try {
      await apiClient.put(`/attendance/${selected.staff_id}`, {
