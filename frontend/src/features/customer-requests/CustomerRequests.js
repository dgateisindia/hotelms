...
const API_BASE_URL = "/customer-requests";
const ROOMS_API_URL = "/rooms/available";

const statusClass = (status) => {
  switch (status) {
    case "pending": return "customer-requests-badge customer-requests-badge--pending";
    case "approved": return "customer-requests-badge customer-requests-badge--approved";
    case "declined": return "customer-requests-badge customer-requests-badge--declined";
    default: return "customer-requests-badge";
  }
};
...
const CustomerRequests = () => {
  ...
  const loadAvailableRooms = async (requestId, checkIn, checkOut) => {
    ...
    try {
      const res = await apiClient.get(ROOMS_API_URL, {
  ...
  const markSeen = async (id) => {
    try {
      await apiClient.patch(`${API_BASE_URL}/${id}/seen`);
      setRequests((prev) =>
  ...
  const handleApprove = async (id) => {
    ...
    const roomId = Number(roomInputs[id]);

    if (!request) {
      alert("Customer request could not be found.");
      return;
    }
    ...
    try {
      ...
      await apiClient.post("/bookings", {
        ...
        special_request: request.specialRequest || null,
        rooms: [{
          room_id: roomId,
          stay_type: "overnight",
          check_in: request.checkIn,
          check_out: request.checkOut,
          total_guests: Number.isFinite(guestCount) && guestCount >= 0 ? guestCount : 0,
        }],
      });
      ...
      alert(getApiErrorMessage(err, "Failed to convert request into booking."));
    } finally {
      setActingId(null);
    }
  };
  ...
  const handleDecline = async (id) => {
    ...
    try {
      setActingId(id);
      const res = await apiClient.patch(`${API_BASE_URL}/${id}/decline`);
  ...
  if (loading) return <div className="customer-requests-card">Loading requests…</div>;
  if (error) return <div className="customer-requests-card">{error}</div>;

  return (
    <div className="customer-requests-card">
      <div className="customer-requests-card-header">
        <span className="customer-requests-card-title">Customer Room Requests</span>
      </div>

      {requests.length === 0 ? (
        <p style={{ padding: "16px" }}>No requests yet.</p>
      ) : (
        <table className="customer-requests-table">
          <thead>
            <tr>
              <th>Customer</th>
              <th>Contact</th>
                ...
                <td>
                  {r.status === "pending" ? (
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      ...
                      <button
                        className="customer-requests-btn customer-requests-btn--primary"
                    ...
                    <span style={{ fontSize: 12, color: "#16a34a" }}>
                      Room {r.assignedRoomNumber || r.assignedRoomId}
                    </span>
                  ) : (
